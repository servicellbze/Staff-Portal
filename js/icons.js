// ServiCell Staff Portal — shared SVG icons (stroke style, matches nav bell)
(function () {
    const PATHS = {
        ...
        crown: '<path d="M2 19h20"/><path d="M4 15l2-8 4 4 2-6 2 6 4-4 2 8"/>',
        ...
        sunrise: '<path d="M17 18a5 5 0 0 0-10 0"/><line x1="12" y1="2" x2="12" y2="9"/><line x1="4.22" y1="10.22" x2="5.64" y2="11.64"/><line x1="1" y1="18" x2="3" y2="18"/><line x1="21" y1="18" x2="23" y2="18"/><line x1="18.36" y1="11.64" x2="19.78" y2="10.22"/>',
        ...
        templates: '<polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>',
        revert: '<polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/>',
        job: '<rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/>',
        drawer: '<rect x="2" y="4" width="20" height="16" rx="2"/><circle cx="12" cy="12" r="2"/>',
        ...
        dotGreen: '<circle cx="12" cy="12" r="10" fill="currentColor" stroke="none"/>',
    ...
    function scIcon(name, size, className) {
        const paths = PATHS[name];
        if (!paths) return '';
        const sz = size || 16;
        const cls = className || 'sc-icon';
        return '<svg class="' + cls + '" width="' + sz + '" height="' + sz + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' + paths + '</svg>';
    }

    function scIconLabel(name, text, size) {
        return scIcon(name, size || 16) + '<span>' + text + '</span>';
    }

    function initDataIcons(root) {
        const scope = root || document;
        scope.querySelectorAll('[data-icon]').forEach(function (el) {
            const name = el.dataset.icon;
            if (!name) return;
            const size = parseInt(el.dataset.iconSize || '16', 10);
            const only = el.classList.contains('empty-icon') || el.classList.contains('state-icon') || el.classList.contains('delete-icon');
            const svg = scIcon(name, size, el.dataset.iconClass || 'sc-icon');
            if (!svg) return;
            if (only) {
                el.innerHTML = svg;
            } else if (!el.querySelector('.sc-icon')) {
                el.insertAdjacentHTML('afterbegin', svg);
            }
        });
    }

    window.SC_ICON_PATHS = PATHS;
    window.scIcon = scIcon;
    window.scIconLabel = scIconLabel;
    window.initDataIcons = initDataIcons;
})();
