...
function printEOD() {
    ...
    const html = '<!DOCTYPE html><html><head><title>EOD Report</title>'
        ...
        + 'body{font-family:"Courier New",Courier,monospace;font-size:14pt;font-weight:bold;width:72mm;margin:0 auto;padding:3mm 3mm 15mm 3mm;color:#000;background:#fff;}'
        + 'h2{text-align:center;font-size:16pt;font-weight:900;margin:0 0 2mm;letter-spacing:1px;}'
        + 'p{text-align:center;margin:0 0 1mm;font-size:13pt;font-weight:bold;}'
        ...
        + '.total td{border-top:3px solid #000;border-bottom:none;font-size:15pt;font-weight:900;padding-top:4px;}'
        + '.variance td{font-size:14pt;font-weight:900;}'
        + '.footer{text-align:center;font-size:12pt;font-weight:bold;margin-top:3mm;border-top:1px dashed #000;padding-top:2mm;}'
        + '</style></head><body>'
        ...
        + (allPayouts.length ? allPayouts.map(p => '<tr><td style="font-size:12pt;">&nbsp;&nbsp;' + escH(p.reason || 'Payout') + (p.takenBy ? ' (' + escH(p.takenBy) + ')' : '') + '</td><td style="font-size:12pt;">-' + bz(p.amount) + '</td></tr>').join('') : '')
...
function printSalesReport() {
    ...
    const salesRows = [...validSales].reverse().map(s => {
        ...
        const itemRows = items.map(item => {
            ...
            return '<tr><td style="font-size:12pt;">' + itemName + '</td>'
                + '<td style="text-align:center;font-size:12pt;">' + qty + '</td>'
                + '<td style="text-align:right;font-size:12pt;">' + bz(total) + '</td></tr>';
        }).join('');
        
        // Header row for this sale
        const headerRow = '<tr style="background:#f0f0f0;"><td colspan="3" style="font-size:12pt;padding:2px 0;border-top:1px solid #000;">'
    ...
    const html = '<!DOCTYPE html><html><head><title>Sales Report</title>'
        ...
        + '<p>Sales Report</p>'
        + '<p>' + displayDate + '</p>'
        + (shift ? '<p style="font-size:12pt;">' + shift.label + '</p>' : '')
        + '<hr>'
        + '<table>'
        + '<tr><th style="text-align:left;font-size:12pt;padding:2px 0;border-bottom:2px solid #000;">Item</th>'
        + '<th style="text-align:center;font-size:12pt;padding:2px 0;border-bottom:2px solid #000;">Qty</th>'
        + '<th style="text-align:right;font-size:12pt;padding:2px 0;border-bottom:2px solid #000;">Total</th></tr>'
        + salesRows
...
function printPayoutsReport() {
    ...
    const payoutRows = [...allPayouts].reverse().map(p => {
        const ts = p.timestamp ? new Date(p.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';
        return '<tr><td style="font-size:12pt;">' + escH(ts) + '</td><td style="font-size:12pt;">' + escH((p.reason || 'Payout').substring(0, 30)) + '</td><td style="font-size:12pt;">' + escH(p.takenBy || ' · ') + '</td><td style="text-align:right;font-size:12pt;">' + bz(p.amount) + '</td></tr>';
    }).join('');
    ...
    const html = '<!DOCTYPE html><html><head><title>Payouts Report</title>'
        ...
        + '<hr>'
        + '<table>'
        + '<tr><th style="text-align:left;font-size:12pt;padding:2px 0;border-bottom:2px solid #000;">Time</th><th style="text-align:left;font-size:12pt;padding:2px 0;border-bottom:2px solid #000;">Reason</th><th style="text-align:left;font-size:12pt;padding:2px 0;border-bottom:2px solid #000;">Taken By</th><th style="text-align:right;font-size:12pt;padding:2px 0;border-bottom:2px solid #000;">Amount</th></tr>'
        + (payoutRows || '<tr><td colspan="4" style="text-align:center;font-size:12pt;padding:8px 0;">No payouts</td></tr>')
...
function printBillsReport() {
    ...
    const billRows = open.map(b => {
        ...
        return '<tr><td style="font-size:12pt;">' + escH(b.personName || 'Unknown') + '</td><td style="font-size:12pt;">' + escH(itemNames.substring(0, 25)) + (itemNames.length > 25 ? '...' : '') + '</td><td style="text-align:right;font-size:12pt;">' + bz(balance) + '</td></tr>';
    }).join('');
    ...
    const html = '<!DOCTYPE html><html><head><title>Bills Report</title>'
        ...
        + '<hr>'
        + '<table>'
        + '<tr><th style="text-align:left;font-size:12pt;padding:2px 0;border-bottom:2px solid #000;">Person</th><th style="text-align:left;font-size:12pt;padding:2px 0;border-bottom:2px solid #000;">Items</th><th style="text-align:right;font-size:12pt;padding:2px 0;border-bottom:2px solid #000;">Balance</th></tr>'
        + (billRows || '<tr><td colspan="3" style="text-align:center;font-size:12pt;padding:8px 0;">No open bills</td></tr>')
